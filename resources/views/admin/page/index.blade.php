...
<section class="py-4">
	<div class="row align-items-center justify-content-center">
		<div class="col-6">
			<h4 class="fw-bolder m-0">Pages</h4>
		</div>
		<div class="col-6">
					...
					<div class="card-header border-0 bg-transparent p-4">
						<div class="d-flex justify-content-between">
							...
							<div class="d-flex align-items-center">
								<a class="btn btn-sm btn-gradient-primary" href="{{ url('/admin/page/create') }}"><i class="bi-plus text-white"></i> Create</a>
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	...
	function initTable(){
		...
		data_table.on("click", "#delete-page", function(){
			var _this = this;
			confirmDelete("Do you really want to delete this page?",
				function(){
					deletePage(data_table, _this.dataset.id);
				},
				function(){}
			);
		});
		...
		document.getElementById("bulk-action-btn").addEventListener("click", function(){
			...
			case 'pending':
			case 'published':
				pageStatus(data_table, bulkAction.value);
				break;
			case 'delete':
				confirmDelete("Are you sure you want to delete selected pages?<br> This cannot be undone.",
					function(){
						deletePage(data_table, 0, true);
					},
					function(){}
				);
				break;
	...
	async function pageStatus(table, status){
		var idArr = [];
		...
		try{
			const response = await fetch("{{url('/admin/page/status/edit')}}",{
				method: "POST",
				mode: "cors",
				cache: "no-cache",
	...
	async function deletePage(table, id = 0, isBulk = false){
		var idArr = [];
		...
		try{
			const response = await fetch("{{url('/admin/page/delete')}}",{
				method: "POST",
				mode: "cors",
				cache: "no-cache",
				credentials: "same-origin",
