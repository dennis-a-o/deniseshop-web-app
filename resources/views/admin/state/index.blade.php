...
<section class="py-4">
	<div class="row align-items-center justify-content-center">
		<div class="col-6">
			<h4 class="fw-bolder m-0">States</h4>
		</div>
		<div class="col-6">
					...
					<div class="card-header border-0 bg-transparent p-4">
						<div class="d-flex justify-content-between">
							...
							<div class="d-flex align-items-center">
								<a class="btn btn-sm btn-gradient-primary" href="{{ url('/admin/state/create') }}"><i class="bi-plus text-white"></i> Create</a>
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	...
	function initTable(){
		...
		data_table.on("click", "#delete-item", function(){
			var _this = this;
			confirmDelete("Do you really want to delete this state?",
				function(){
					deleteState(data_table, _this.dataset.id);
				},
				function(){}
			);
		});
		...
		document.getElementById("bulk-action-btn").addEventListener("click", function(){
			...
			case 'pending':
			case 'published':
				stateStatus(data_table, bulkAction.value);
				break;
			case 'delete':
				confirmDelete("Are you sure you want to delete selected states?<br> This cannot be undone.",
					function(){
						deleteState(data_table, 0, true);
					},
					function(){}
				);
				break;
	...
	async function stateStatus(table, status){
		var idArr = [];
		...
		try{
			const response = await fetch("{{url('/admin/state/status/edit')}}",{
				method: "POST",
				mode: "cors",
				cache: "no-cache",
	...
	async function deleteState(table, id = 0, isBulk = false){
		var idArr = [];
		...
		try{
			const response = await fetch("{{url('/admin/state/delete')}}",{
				method: "POST",
				mode: "cors",
				cache: "no-cache",
				credentials: "same-origin",
